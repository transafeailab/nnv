function example_nonlinear_reach_ARCH17_vanDerPol_pseudoInvariant()
% example_nonlinear_reach_ARCH17_vanDerPol_pseudoInvariant - example of nonlinear reachability 
% analysis
%
% Syntax:  
...
%first guard set
guard1 = interval([1.5-smallDist; -2],[1.5; 0]);
%resets
reset1.A = eye(2); 
reset1.b = zeros(2,1);
...
for iMode = 1:length(Rplot)
    for iSet = 1:length(Rplot{iMode})
        Rtmp = reduce(project(Rplot{iMode}{iSet},[1 2]),'girard',redOrder);
