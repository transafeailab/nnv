...
%for iSys = 1:length(index)
    ...
    %obtain hessian tensor; IMPORTANT: use modified INTLAB for parallel
    %processing due to sin/cos evaluation using global parameters
    [Hf, Hg] = feval(subsystem{iSys}.hessian, totalInt_x, totalInt_y, totalInt_u);
    %[Hf, Hg] = subsystem{iSys}.hessian(totalInt_x, totalInt_y, totalInt_u);
...
errorInt = supremum(errorIHabs(:,2));

errorIHabs_y = abs(interval(errorTotal_y));
errorInt_y = supremum(errorIHabs_y(:,2));

errorIHabs_x = abs(interval(errorTotal_x));
errorInt_x = supremum(errorIHabs_x(:,2));
