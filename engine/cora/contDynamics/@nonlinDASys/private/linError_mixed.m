function [error, errorInt, errorInt_x, errorInt_y, R_y] = linError_mixed(obj, options, R, Verror_y)
...
totalInt_y = d_y + obj.linError.p.y;

%compute intervals of input
totalInt_u = d_u + obj.linError.p.u;
...
%zonotope evaluation
error_x_mid = 0.5*quadraticMultiplication(R_xyu, Hf_mid);
error_y_mid = 0.5*quadraticMultiplication(R_xyu, Hg_mid);

%interval evaluation
for i=1:length(Hf)
    error_x_rad(i,1) = 0.5*dz'*Hf_rad{i}*dz;
end
for i=1:length(Hg)
    error_y_rad(i,1) = 0.5*dz'*Hg_rad{i}*dz;
end

%combine results
error_x_rad_zono = zonotope(interval(infimum(error_x_rad), supremum(error_x_rad)));
error_y_rad_zono = zonotope(interval(infimum(error_y_rad), supremum(error_y_rad)));
